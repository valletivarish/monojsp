...
public class StudentController extends HttpServlet {
	...
	private void searchStudent(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if (searchType != null && search != null && !search.isEmpty()) {
	        switch (searchType) {
	            case "rollnumber":
	                int rollnumber = Integer.parseInt(search);
	                students = studentDbUtil.getStudentSearchOnRollnumber(rollnumber);
	                break;
	            case "name":
	                students = studentDbUtil.getStudentSearchOnName(search);
	                break;
	            case "percentage":
	                int percentage = Integer.parseInt(search);
	                students = studentDbUtil.getStudentSearchOnPercentage(percentage);
	                break;
	            default:
	                students = studentDbUtil.getStudentList();
	        }
	    } else {
	        students = studentDbUtil.getStudentList();
	    }
